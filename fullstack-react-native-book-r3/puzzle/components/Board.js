...
export default class Board extends React.PureComponent {
  ...
  async componentWillReceiveProps(nextProps) {
    ...
    const shouldTransitionOut = !this.props.teardown && teardown;

    if (this.props.puzzle !== puzzle && previousMove !== null) {
    ...
    return (
      ...
      >
        {({ handlers, dragging }) => {
          ...
          const imageStyle = {
            position: 'absolute',
            top: -Math.floor(square / size) * (itemSize + itemMargin),
            left: -Math.floor(square % size) * (itemSize + itemMargin),
